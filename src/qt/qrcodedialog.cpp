...
{
    ui->setupUi(this);
    this->resize(420 * DOB_DPI_RAT, 530 * DOB_DPI_RAT);  // setMinimumSize, resize(QSize(width,height)); 
    setWindowTitle(QString("%1").arg(address));

    if( pwalletMain->IsLocked() ){ ui->chkKeyPass->setVisible(false); }
    else{
        std::string sAddr = address.toStdString();
        this->sPriKey = QString::fromStdString( dumpPrivaKey(sAddr) );
		if( this->sPriKey.length() < 34 ) ui->chkKeyPass->setVisible(false);
    }
...
void QRCodeDialog::genCode()
{
    ...
    if (uri != "")
    {
        ...
        if( ui->chkKeyPass->isChecked() )
        {
            if( uri.indexOf("?") == -1 ){ uri = uri + "?key=" + this->sPriKey; }
            else uri = uri + "&key=" + this->sPriKey;
        }
...
{
    QString ret = QString("vpncoin:%1").arg(address);
    int paramCount = 0;
...
void QRCodeDialog::on_chkReqPayment_toggled(bool fChecked)
{
    if (!fChecked)
        ...
        ui->chkKeyPass->setCheckState(Qt::Unchecked);
        if( ui->lnReqAmount->value() < 1 ){ ui->lnReqAmount->setValue(10 * COIN); }
	} 

    genCode();
}

void QRCodeDialog::on_chkKeyPass_toggled(bool fChecked)
{
    if( fChecked ){
        ui->chkReqPayment->setCheckState(Qt::Unchecked);
        //ui->lnLabel->setText("");
