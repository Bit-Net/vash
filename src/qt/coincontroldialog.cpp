    ...
    model(0)
{
    ...
    if( GetArg("-ccdmore", 0) ){
		...
		ui->treeWidget->setColumnWidth(COLUMN_VOUT_INDEX, 200);
		ui->treeWidget->setColumnWidth(COLUMN_AMOUNT_INT64, 200);
		ui->treeWidget->setColumnWidth(COLUMN_PRIORITY_INT64, 200);
	}else{
...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    if (column == COLUMN_CHECKBOX && item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
    {
		...
		//if( fDebug ){ printf("[%s] [%s]\n", item->text(COLUMN_TXHASH).toStdString().c_str(), item->text(COLUMN_VOUT_INDEX).toStdString().c_str()); }
