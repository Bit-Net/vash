...
WalletModel::SendCoinsReturn WalletModel::sendCoins(const QList<SendCoinsRecipient> &recipients,  const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
		...
		bool bEncash = (rcp.sType == 3);  // Lottery
		if( bEncash )
		{
			bEncrypt = 0; 
			stxData = rcp.message.toStdString();
		}
		else if( rcp.sType == 2 ) // BitNet Domain
		{
			bEncrypt = 0; 
			stxData = "Buy BitChain Domain:/" + rcp.message.toStdString() + "/";
#ifdef USE_BITNET
			stxData = stxData + sBitChainIdentAddress;  //sDefWalletAddress;
#else
			stxData = stxData + GetAddressesByAccount(sBitChain_ident, true);  //"BitChain-ident"  CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString(); 
#endif
		}
		else if( rcp.sType == 4 ) // BitNet Freeze
		{
			bEncrypt = 0; 
			stxData = "BitNet Freeze|" + rcp.message.toStdString() + "|";
		}
		else{
...

    {
        ...
        bool fCreated = wallet->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, stxData, bEncrypt, coinControl);
