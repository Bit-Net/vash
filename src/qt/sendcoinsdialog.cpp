    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a VpnCoin address (e.g. Vvpywwpq4uKGvouhNv7jzKZ4uoYm3ECsbM)"));
#endif
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    case WalletModel::TransactionCommitFailed:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            QMessageBox::Ok, QMessageBox::Ok);
        break;
    case WalletModel::InvalidMessage:
        QMessageBox::warning(this, tr("Send Coins"),
            tr("The message contains illegal characters."),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
    case WalletModel::InvalidMessageAmount:
        QMessageBox::warning(this, tr("Send Coins"),
            tr("Your have to send at least 1 VPN to write a message to recipient."),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
bool SendCoinsDialog::handleURI(const QString &uri, int bTip)
{
    SendCoinsRecipient rv;
    // URI has to be valid
	//if( fDebug ){ printf("SendCoinsDialog::handleURI [%s]\n", uri.toStdString().c_str()); }
    if (GUIUtil::parseBitcoinURI(uri, &rv))
    {
        //if( fDebug ){ printf("SendCoinsDialog::handleURI address = [%s]\n", rv.address.toStdString().c_str()); }
		CBitcoinAddress address(rv.address.toStdString());
        if (!address.IsValid())
		{
            //if( fDebug ){ printf("SendCoinsDialog::handleURI [%s] address not Valid\n", rv.address.toStdString().c_str()); }
			return false;
		}
			
		// 2015.02.01 BitNet Add
		//CTxDestination dest = address.Get();
		//if( IsMine(*pwalletMain, dest) ){ return false; }
		
		if( bTip > 0 )	// 2015.07.22 add
		{
			int i = GetArg("-maxtipamount", 100000);
			if( i > 0 )
			{
				int64_t i6 = i * COIN;
				if( rv.amount > i6 ){ rv.amount = i6; }
			}
		}
        pasteEntry(rv);
        return true;
    }//else{ if( fDebug ){ printf("SendCoinsDialog::handleURI [%s] false\n", uri.toStdString().c_str()); } }
...
{
    if( GetArg("-hidebalance", 0) ){ return; }
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid VpnCoin address"));
        }
        else
        {
