...
void MultisigAddressEntry::on_pubkey_textChanged(const QString &pubkey)
{
    if( pubkey.length() < 34 )
    {
		ui->address->clear();     ui->label->clear();     return;
    }
...
void MultisigAddressEntry::on_address_textChanged(const QString &address)
{
    ...
    {
		ui->pubkey->clear();     ui->label->clear();     return;
    }
    ...
    {
        CPubKey vCPubKey;
        model->getPubKey(keyID, vCPubKey);
        std::vector<unsigned char> vchPubKey = vCPubKey.Raw();
