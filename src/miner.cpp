...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake, int64_t* pFees)
{
    ...
    unsigned int nBlockMaxSize = GetArg("-blockmaxsize", max_BLOCK_SIZE_GEN(nBestHeight)/2);  //MAX_BLOCK_SIZE_GEN
    ...
    int64_t nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            bool bZkpMintTx = false;
			if( IsZkpRuleActived(nBestHeight) ){ bZkpMintTx=isZkpMintTx(tx); }
			if( bZkpMintTx && !isValidZkpMintTx(tx, "CreateNewBlock()", nBestHeight, false, false, false, false) )
			{
                delZeroKnowledgeProofMintTxFromDB(tx);      mempool.remove(tx);     continue;
			}
            // Transaction fee
            int64_t nMinFee = tx.GetMinFee(nBlockSize, GMF_BLOCK, bZkpMintTx);
            ...
            if( (bZkpMintTx == false) && !tx.ConnectInputs(txdb, mapInputs, mapTestPoolTmp, CDiskTxPos(1,1,1), pindexPrev, false, true) )
                continue;
