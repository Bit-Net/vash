...
public:
    ...
    bool CreateTransaction(const std::vector<std::pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, std::string& stxData, int bEncrypt, const CCoinControl *coinControl=NULL);
    ...
    std::string SendMoney(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, std::string& stxData, int bEncrypt, bool fAskFee=false);
    std::string SendMoney(const vector<pair<CScript, int64_t> >& vecSend, int64_t nValue, CWalletTx& wtxNew, std::string& stxData, int bEncrypt , bool fAskFee=false);
    std::string SendMoneyToDestination(const CTxDestination &address, int64_t nValue, CWalletTx& wtxNew, std::string& stxData, int bEncrypt=1, bool fAskFee=false);
    ...
    {
        bool b = ::IsMine(*this, txout.scriptPubKey);
		if( !b )
		{
			CTxDestination address;   txnouttype txotp;
			if ( ExtractDestination(txout.scriptPubKey, address, &txotp) )
			//if (ExtractDestination(txout.scriptPubKey, address) && ::IsMine(*this, address))
			{
				if( TX_SCRIPTHASH == txotp ){ b = ::IsMine(*this, address); }
				//if( fDebug ){ printf("IsMine():: txOut To [%s], Value [%I64u], IsMine = [%d], type = [%d] \n", CBitcoinAddress(address).ToString().c_str(), txout.nValue, b, txotp); }
			}
		}
        return b;  //::IsMine(*this, txout.scriptPubKey);
    }
