...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, Object& out, bool fIncludeHex)
{
    ...
    if (fIncludeHex || GetArg("-pushspubkeyhextojson", 0))
...
string getrawtransaction(const string txID)
{
	...
	if( txID.length() > 34 )
	{
		...
		ssTx << tx;
		rzt = HexStr(ssTx.begin(), ssTx.end());
	}
	return rzt;
}
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid VpnCoin address: ")+input.get_str());
