...
const uint256 hashGenesisBlock("0x00000ac7d764e7119da60d3c832b1d4458da9bc9ef9d5dd0d91a15f690a46d99");
const uint256 hashGenesisBlockTestNet("0x00003a4179371d1eabacf71f0cd1e4e071cda2bea1a7e01d4260dc07eead9d73");
...
bool CTransaction::CheckTransaction(bool bZkpMintTx, bool bChkMiniValue) const
{
    // Basic checks that don't depend on any context
    if ( !bZkpMintTx && vin.empty())
...
int64_t CTransaction::GetMinFee(unsigned int nBlockSize, enum GetMinFee_mode mode, unsigned int nBytes, bool bZkpMintTx) const
{
    if( bZkpMintTx ){ return 0; }
...
//bool isValidZkpMintTx(const CTransaction& tx, const string sCallFrom, int nHeight, bool bJustRecvTx, bool bSaveMintKeyToDb, bool bUpdateSpentToDb, bool bInOtherChain, bool bJustCheck=false);
	if( IsZkpRuleActived(nBestHeight) && isZkpMintTx(tx) )
	{
		bValidZkpMintTx = isValidZkpMintTx(tx, "AcceptToMemoryPool()", nBestHeight, true, true, false, false);
		if( !bValidZkpMintTx ){ return error("AcceptToMemoryPool : Invalid Zkp Mint Tx"); }
	}

    if ( !tx.CheckTransaction(bValidZkpMintTx) )
...

    {
        ...
        int64_t txMinFee = tx.GetMinFee(1000, GMF_RELAY, nSize, bValidZkpMintTx);
		if( fDebug ){ printf("AcceptToMemoryPool : bValidZkpMintTx=[%d], txMinFee=[%s] \n", bValidZkpMintTx, i64tostr(txMinFee).c_str()); }
		if( !bValidZkpMintTx ){
        if (nFees < txMinFee)
        {
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    int64_t nValueIn = 0;
    int64_t nValueOut = 0;
    int64_t nStakeReward = 0,  nZkpBurndCoins=0;
    unsigned int nSigOps = 0;
    BOOST_FOREACH(CTransaction& tx, vtx)
    {
        uint256 hashTx = tx.GetHash();

		bool bZkpMintTx = isZkpMintTx(tx);
        ...
        else
        {
            ...
            if ( (!bZkpMintTx) && !tx.ConnectInputs(txdb, mapInputs, mapQueuedChanges, posThisTx, pindex, true, false) )
                return false;
        }
	...
	if( nZkpBurndCoins > 0 ){ pindex->nMoneySupply = pindex->nMoneySupply - nZkpBurndCoins; }   // 2017.02.28 add
...
bool static Reorganize(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    for (unsigned int i = 0; i < vConnect.size(); i++)
    {
		...
		int nBlkHei = pindex->nHeight;   bool bCheckZkpMintTxOk = true, bZkpActived = IsZkpRuleActived(nBlkHei);
		if( bZkpActived )
		{
			BOOST_FOREACH(CTransaction& tx, block.vtx)
			{
		// bool isValidZkpMintTx(const CTransaction& tx, const string sCallFrom, bool bJustRecvTx, bool bSaveMintKeyToDb, bool bUpdateSpentToDb, bool bInOtherChain, bool bJustCheck=false)
				if( isZkpMintTx(tx) )
				{
					if( !isValidZkpMintTx(tx, "Reorganize( pre check zkp )", nBlkHei, false, false, false, false) )
					{
						bCheckZkpMintTxOk = false;      break;
					}
				}
			}
		}

        if ( !bCheckZkpMintTxOk || !block.ConnectBlock(txdb, pindex) )
        {
            // Invalid block
...
bool CBlock::SetBestChainInner(CTxDB& txdb, CBlockIndex *pindexNew)
{
	...
	int nBlkHei = pindexNew->nHeight;     bool bCheckZkpMintTxOk = true, bZkpActived = IsZkpRuleActived(nBlkHei);
    if( bZkpActived )
	{
		BOOST_FOREACH(CTransaction& tx, vtx)
		{
	// bool isValidZkpMintTx(const CTransaction& tx, const string sCallFrom, bool bJustRecvTx, bool bSaveMintKeyToDb, bool bUpdateSpentToDb, bool bInOtherChain, bool bJustCheck=false)
			if( isZkpMintTx(tx) )
			{
				if( !isValidZkpMintTx(tx, "SetBestChainInner( pre check zkp )", nBlkHei, false, false, false, false) )
				{
					bCheckZkpMintTxOk = false;      break;
				}
			}
    ...
    if ( !bCheckZkpMintTxOk || !ConnectBlock(txdb, pindexNew) || !txdb.WriteHashBestChain(hash))
    {
        txdb.TxnAbort();
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    // Check transactions
	bool bZkpActived = IsZkpRuleActived(nBestHeight);
    BOOST_FOREACH(const CTransaction& tx, vtx)
    {
        //string txHash = tx.GetHash().ToString();
		bool bValidZkpMintTx=false;
		if( bZkpActived ){ bValidZkpMintTx = isZkpMintTx(tx); }
		/*if( isZkpMintTx(tx) )   // bool isValidZkpMintTx(const CTransaction& tx, bool bJustRecvTx, bool bSaveMintKeyToDb, bool bUpdateSpentToDb)
		{
			bValidZkpMintTx = isValidZkpMintTx(tx, "CheckBlock()", false, false, false);
			if( !bValidZkpMintTx ){ return error("CheckBlock() : Invalid Zkp Mint Tx [%s]", txHash.c_str()); }
		}*/
        if ( !tx.CheckTransaction(bValidZkpMintTx) )
...
//-- 2015.04.27 add, Check for BitNet Lottery end
		...
		bool bValidZkpMintTx=false;      string txHash = tx.GetHash().ToString();
		if( bZkpActived && isZkpMintTx(tx) )   // bool isValidZkpMintTx(const CTransaction& tx, bool bJustRecvTx, bool bSaveMintKeyToDb, bool bUpdateSpentToDb)
		{
			bValidZkpMintTx = isValidZkpMintTx(tx, "AcceptBlock()", nHeight, false, false, false, bInOtherChain);
			if( !bValidZkpMintTx ){ return error("AcceptBlock() : Invalid Zkp Mint Tx [%s]", txHash.c_str()); }
		}
	}
...
bool LoadBlockIndex(bool fAllowNew)
{
	...
	int iChangeToTest = GetArg("-chgtotest", 0);
    if ( fTestNet || (iChangeToTest > 0) )
    {
        pchMessageStart[0] = 0xcd;
        pchMessageStart[1] = 0xf2;
        pchMessageStart[2] = 0xc0;
        pchMessageStart[3] = 0xef;
		BitNet_Network_id = 0x0df2c0ef;
    ...
    if (mapBlockIndex.empty())
    {
		...
		if (fTestNet || (iChangeToTest > 0) )
		{
			...
			block.nNonce   = 2275921;  //2263742;
		}

		uint256 aHash = (!fTestNet ? hashGenesisBlock : hashGenesisBlockTestNet);
        //// debug print
        uint256 hash = block.GetHash();
        printf("block.GetHash() =%s\n", hash.ToString().c_str());
        printf("hashGenesisBlock =%s\n", aHash.ToString().c_str());
		...
		uint256 aMerkle = uint256("0x698a93a1cacd495a7a4fb3864ad8d06ed4421dedbc57f9aaad733ea53b1b5828");
		if( fTestNet || (iChangeToTest > 0) ){ aMerkle = uint256("0x23de3a1724ab2c7193f4d0ff6cf1d5b745db9dc0af7fc796db9b3494da42a7c9"); }
		assert(block.hashMerkleRoot == aMerkle);
        /*if (fTestNet){
        ...
        if (true && block.GetHash() != aHash)
        {
			...
			char scratchbuff[SCRYPT_BUFFER_SIZE2];
