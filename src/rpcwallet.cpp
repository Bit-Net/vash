...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    if( !fTestNet && (mn >= (1000000000 * COIN)) ){ mn = mn - (600000000 * COIN); }
...
int isSoCoinAddress(const CTransaction& tx, const string sAddr, int& iPos)
{
	...
	if( sAddr.length() > 10 )
	{
		...
		string sBaddr = ba;      sBaddr = strprintf("%s%s", sBaddr.c_str(), sZeroKnowledgeProofPoolAddress.c_str());
...
extern int updateAllAdressDB(const string sAddr, int64_t nValue, bool bAdd);
extern void updateTotalCoin(int64_t nValue);
int explainBlockTx(int nHeight)
{
    int rzt = 0;
	if( isBlockProcessed(nHeight) )
	{
		if( fDebug ) printf("\n\n<--- explainBlockTx:: Height = [%d], Processed\n\n", nHeight);
		return rzt;
	}
    CBlockIndex* pblockindex = FindBlockByHeight(nHeight);
	if( fDebug ) printf("\n\n--->explainBlockTx:: Height = [%d], pblockindex = [%x]\n", nHeight, pblockindex);
	if( pblockindex )
	{
		CBlock block;
		block.ReadFromDisk(pblockindex);
		//int nHeight = pblockindex->nHeight;
		int64_t ttInputCoin=0, ttOutCoin=0;   int txIdx = 0;
		if( fDebug ) printf("-->explainBlockTx:: Height = [%d], total tx = [%d]\n", nHeight, block.vtx.size());
		int64_t aTxInCoins = 0, aTxOutCoins = 0;
		BOOST_FOREACH(const CTransaction& tx, block.vtx)
		{
			//if (!IsFinalTx(tx, nHeight, block.GetBlockTime()))
			//	return rzt;
			bool bCoinBase = tx.IsCoinBase(), bCoinStake = tx.IsCoinStake();
			if( fDebug ) printf( "explainBlockTx: BOOST_FOREACH(1), tx(%d) [%s],  IsCoinBase(%d), IsCoinStake(%d)\n", txIdx, tx.GetHash().ToString().c_str(), bCoinBase, bCoinStake );

            int j = 0;
			if( fDebug ) printf( "explainBlockTx: process txin, size = (%d) \n", tx.vin.size());
			BOOST_FOREACH(const CTxIn& txin, tx.vin)
    		{
        		uint256 hashBlock = 0;
        		CTransaction txPrev;
        		...
        		if( fDebug ) printf( "explainBlockTx: BOOST_FOREACH(2), tx.vin(%u) txin.prevout.n = [%u], \n", j, txin.prevout.n );
				if( (unsigned int) -1 != txin.prevout.n ){
				if( GetTransaction(txin.prevout.hash, txPrev, hashBlock) )	// get the vin's previous transaction
				{
					string sPrevTxMsg = txPrev.vpndata;
					CTxDestination source;
					...
					string sPreTxHash = txPrev.GetHash().ToString();
					int iPreTxHei = GetTransactionBlockHeight(sPreTxHash);
					int64_t nValue = txPrev.vout[txin.prevout.n].nValue;
					aTxInCoins = aTxInCoins + nValue;
					if( fDebug ) printf( "explainBlockTx: GetTransaction ok, tx.vin(%u) iPreTxHei = [%u],  nValue [%I64u], sPreTxHash [%s] \n", j, iPreTxHei, nValue, sPreTxHash.c_str() );
					if( nValue > 0 )
					{
						ttInputCoin = ttInputCoin + nValue;
						if (ExtractDestination(txPrev.vout[txin.prevout.n].scriptPubKey, source))  
						{
        	        		CBitcoinAddress addressSource(source);
							preTxAddress = addressSource.ToString();
							int k = updateAllAdressDB(preTxAddress, nValue, false);
							if( fDebug ) printf("explainBlockTx: preTxAddress is [%s] : [%s], k=[%d]\n", preTxAddress.c_str(), sPrevTxMsg.c_str(), k);
						}else{ printf( "explainBlockTx: ExtractDestination(txPrev.vout[txin.prevout.n].scriptPubKey...) faile, \n"); }
					}
				}else{ printf( "explainBlockTx: GetTransaction(txin.prevout.hash...) faile, \n"); }}
			}

			rzt = 1;
			if( fDebug ) printf( "explainBlockTx: process txout, size (%d)\n", tx.vout.size());
			for (unsigned int i = 0; i < tx.vout.size(); i++)
			{
				const CTxOut& txout = tx.vout[i];
				int64_t nValue = txout.nValue;
				//bool bOk = false;
				ttOutCoin = ttOutCoin + nValue;  //txout.nValue;
				aTxOutCoins = aTxOutCoins + nValue;
				if( fDebug ) printf("explainBlockTx process txout(%d), nValue [%I64u]\n", i, txout.nValue);
				if( nValue > 0 )   //if( bOk ) //if( txout.nValue == v_nValue )
				{	
					txnouttype type;
					vector<CTxDestination> addresses;
					int nRequired;
					if( ExtractDestinations(txout.scriptPubKey, type, addresses, nRequired) )
					{
						if( addresses.size() > 1 ) printf("explainBlockTx process txout(%d), addresses.size() = %d \n", i, addresses.size());
						BOOST_FOREACH(const CTxDestination& addr, addresses)
						{
							string sAa = CBitcoinAddress(addr).ToString();
							int k = updateAllAdressDB(sAa, nValue, true);
							if( fDebug ) printf("explainBlockTx process txout(%d), to addr [%s], k=[%d]\n", i, sAa.c_str(), k);
						}
					}else{ printf( "explainBlockTx: ExtractDestinations(txout....) faile, \n"); }
				}
			}
			//printf("explainBlockTx:: aTxInCoins = [%I64u]  aTxOutCoins = [%I64u]\n", aTxInCoins, aTxOutCoins);
			//if( aTxOutCoins < aTxInCoins ){ printf("explainBlockTx:: aTxOutCoins < aTxInCoins :(, eeeeeerror \n"); }
			rzt = 2;   txIdx++;
		}
		if( fDebug ) printf("explainBlockTx:: aTxInCoins = [%I64u]  aTxOutCoins = [%I64u]\n", aTxInCoins, aTxOutCoins);
		if( aTxOutCoins < aTxInCoins ){ printf("explainBlockTx:: aTxOutCoins [%I64u] < aTxInCoins [%I64u] :(, eeeeeerror \n", aTxOutCoins, aTxInCoins); }
		insertProcessedBlock(nHeight, block.GetHash().ToString());
		if( (ttOutCoin > 0) && (ttOutCoin > ttInputCoin) ){ updateTotalCoin(ttOutCoin - ttInputCoin); }
		if( fDebug ) printf("<--- explainBlockTx:: Height = [%d], ttInputCoin=[%I64u], ttOutCoin=[%I64u] \n\n", nHeight, ttInputCoin, ttOutCoin);
	}
    return rzt;
}

Value explainblocktx(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1)
	{
		throw runtime_error(
            "explainblocktx <start block number> <stop block number>\n");
	}
	//string sAddr = params[0].get_str();
    int iStart = params[0].get_int(), iStop = 0, i = 0;
	if( params.size() > 1 ) iStop = params[1].get_int();
	//int iPort = params[2].get_int();
	Object ret;
	if( (iStop <= 0) || (iStop <= iStart) )
	{
		i =explainBlockTx(iStart);
		ret.push_back( Pair("Result", i) );
	}else{
		int j = 0;
		for( i = iStart; i < (iStop + 1); i++ )
		{
			if( explainBlockTx(i) > 1 ){ j++; }
		}
...
Value zkpburn(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2)
	{
		throw runtime_error(
            "zkpburn <burn coins> <public key for redemption>\nzkpburn 100 VpnZKPAEd1HnSzJqfQ6x2yThdKpEEQA1HA");  //"zkpburn <burn coins> <proof key> <proof type>\nzkpburn 100 VpnZKPAEd1HnSzJqfQ6x2yThdKpEEQA1HA 0");
	}
	if( !IsZkpRuleActived(nBestHeight) ){ throw runtime_error("ZKP rules not actived, please wait"); }
	std::string sBurnCoins = params[0].get_str(), sProofKeys = params[1].get_str();
	string sProofType = "0";   	if( params.size() > 2 ) sProofType = params[2].get_str();
	int iBurnCoins = atoi( sBurnCoins.c_str() ), iProofType = atoi( sProofType.c_str() );
	//int64_t i6Coin = iBurnCoins;
    // Amount
    int64_t nAmount = iBurnCoins * COIN;
	if( nAmount <= Min_Burn_Coin_Amount ){ throw runtime_error("burn coins must big than 99"); }
	if( zkpClassExists(iProofType, nAmount, sProofKeys) )
	{
		throw runtime_error("zpk exists, please change <burn coins> or change <proof key>");
	}
    // Wallet comments
    CWalletTx wtx;
    string stxData = sProofKeys;  // + "|" + sProofType;
	int bEncrypt = 0;
    if (pwalletMain->IsLocked())
        throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Error: Please enter the wallet passphrase with walletpassphrase first.");
	...
	{
		uint256 hash = getZkpHashFromParams(iProofType, nAmount, sProofKeys, true);
		return hash.GetHex();
	}
}

Value zkpinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1)   // zkpinfo 233c7e873cfb0c4a2bc961e0396ebf5b3614434c4c93f51f08753aba390a3b2d
	{
		throw runtime_error("zkpinfo <zkp hash>");   // zkpinfo 233c7e873cfb0c4a2bc961e0396ebf5b3614434c4c93f51f08753aba390a3b2d
	}
	std::string sZkpHash = params[0].get_str();      CZeroKnowledgeProof zkp;
	if( ReadZeroKnowledgeProof(sZkpHash, zkp) )
	{
		Object ret;
		//ret.push_back( Pair("ProofType", zkp.nProofType) );
		ret.push_back( Pair("Coins", ValueFromAmount(zkp.n6Coins)) );
		ret.push_back( Pair("Spent", ValueFromAmount(zkp.n6Spent)) );
		ret.push_back( Pair("EstSpent", ValueFromAmount(zkp.n6EstSpent)) );
		ret.push_back( Pair("Public Key", zkp.GetProofKeys()) );
		ret.push_back( Pair("Status", zkp.nStatus) );
		return ret;
	}else{ throw runtime_error("zpk not exists :("); }
}

Value genzkpmintkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3)   // bool genZkpMintKey(const string& sZkpHash, int64_t i6Amount, const string& sToAddress, std::string& sRztMintKey)
	{
		throw runtime_error(
            "genzkpmintkey <zkp hash> <mint coins> <to address>\genzkpmintkey 0d4d5097e3017ebc12f9f670cfd81dadbe44b35784dee2e5271e14c6f85f3029 100 VieLM7amheaY83qhDyJET57UwwgRgzh41v,VieLM7amheaY83qhDyJET57UwwgRgzh42v");
	}
	std::string sZkpHash = params[0].get_str(), sToAddress = params[2].get_str();  // sMintCoins = params[1].get_str(),
	int64_t iMintCoins = params[1].get_int();   //atoi( sMintCoins.c_str() );
	if( iMintCoins <= 0 ){ throw runtime_error("mint coins must big than zero"); }
    int64_t i6Amount = iMintCoins * COIN;
	string sMintKey="";
	//std::string s;      std::ostringstream oss;      oss<<i6Amount;      s = oss.str();
	//if( fDebug ){ printf("genzkpmintkey:: i6Amount=[%I64d : %s], sToAddress=[%s] \n", i6Amount, int64tostr(iMintCoins).c_str(), sToAddress.c_str()); }
	if( genZkpMintKey(sZkpHash, i6Amount, sToAddress, sMintKey) )
	{
...
Value zkpminttomany(const Array& params, bool fHelp)
{
// zkpminttomany <zkp hash> '{"VZzpuiATQouD4mu9jnedRuKrgpHyY2Me1w":17000000,"VZzpuiATQouD4mu9jnedRuKrgpHyY2Me1w":16800000}' <your sign string>
    if (fHelp || params.size() < 3)
	{
		throw runtime_error(
            "zkpmint <zkp hash> {address:amount,...} <your mint key>\n");
	}

	std::string sZkpHash = params[0].get_str(), sMintKey = params[2].get_str();
    Object sendTo = params[1].get_obj();
    set<CBitcoinAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;
    int64_t totalAmount = 0;
	...
	//int iMintCoins = atoi( sMintCoins.c_str() );
	if( totalAmount <= 0 ){ throw runtime_error("mint coins must big than zero"); }
    //int64_t nAmount = iMintCoins * COIN;
	//std::vector<std::pair<CScript, int64_t> > aVecSend;
	//CScript scriptPubKey;
	//scriptPubKey.SetDestination(CBitcoinAddress(sToAddress).Get());
	//aVecSend.push_back( make_pair(scriptPubKey, nAmount) );
	string sRztTxHash = "";
	zkpmint(sZkpHash, vecSend, sMintKey, sRztTxHash);
	return sRztTxHash;
}

Value zkpmint(const Array& params, bool fHelp)
{
// zkpmint 1ce31617b84cbf109f30359a51cec3d78941a866a9166d8d2e16eb45b1ffbd93 Vdvtbvm7Z9ePrAVUyt82x1LnxZ8FAnxRyL 100 IASf8TaPy3FMmwYm1WqOeakUFwgVbz0hTIRiIZiqrypU9FcAM9TBKvJGowxhaHHSnMt3OCkCktMEaPWypDxenV0=
    if (fHelp || params.size() < 3)
	{
		throw runtime_error(
            "zkpmint <zkp hash> <to address> <mint coins> <your mint key>\n");
	}

	std::string sZkpHash = params[0].get_str(), sToAddress = params[1].get_str(), sMintCoins = params[2].get_str(), sMintKey = params[3].get_str();
	int iMintCoins = atoi( sMintCoins.c_str() );
	if( iMintCoins <= 0 ){ throw runtime_error("mint coins must big than zero"); }
    int64_t nAmount = iMintCoins * COIN;
	std::vector<std::pair<CScript, int64_t> > aVecSend;
	...
	aVecSend.push_back( make_pair(scriptPubKey, nAmount) );
	string sRztTxHash = "";
	zkpmint(sZkpHash, aVecSend, sMintKey, sRztTxHash);
	return sRztTxHash;
}

Value zkpruletest(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2)
	{
		throw runtime_error(
            "zkpruletest <option> <block height>\n");
	}
	std::string sOption = params[0].get_str(), sHei = params[1].get_str();
	int iOpt = atoi( sOption.c_str() ), nHeight = atoi( sHei.c_str() );
	Object ret;
	if( iOpt < 1 )
	{
		string ss = strprintf("IsZkpRuleActived(%d)", nHeight);
		ret.push_back( Pair(ss, IsZkpRuleActived(nHeight)) );
	}else if( iOpt == 1 ){  // int64_t GetMaxZkpMintCoinAmount(int nHeight)
		int64_t i6 = GetMaxZkpMintCoinAmount(nHeight);
		string ss = strprintf("GetMaxZkpMintCoinAmount(%d)", nHeight);
		ret.push_back( Pair(ss, ValueFromAmount(i6)) );
	}else if( iOpt == 2 ){  // bool IsValidBurnOrMintCoinAmount(int nHeight, int64_t i6Amount)
		string sAmt = params[2].get_str();   int64_t i6Amount = atoi( sAmt.c_str() );   i6Amount = i6Amount * COIN;
		string ss = strprintf("IsValidBurnOrMintCoinAmount(%d, %s VASH)", nHeight, int64tostr(i6Amount / COIN).c_str());
		ret.push_back( Pair(ss, IsValidBurnOrMintCoinAmount(nHeight, i6Amount)) );
		//return strprintf("IsValidBurnOrMintCoinAmount(%d, %s VASH) : [%d]", nHeight, int64tostr(i6Amount / COIN).c_str(), IsValidBurnOrMintCoinAmount(nHeight, i6Amount));
	}
	return ret;
}
