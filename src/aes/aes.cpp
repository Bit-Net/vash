...
int AES::Cipher(const string sInput, string& sRzt)
{
    int rzt = 0;     sRzt = "";
	//int64_t i1 = time(NULL);
	int i = sInput.length();
    if( i <= 0 ) return rzt;
	unsigned char* pIn = (unsigned char*)sInput.c_str();
	rzt = i + 4;     int j = i % 16;
	if( j > 0 ){ rzt = rzt + 16 - j; }
	sRzt.resize(rzt);     unsigned char* pOut = (unsigned char*)sRzt.c_str();
	*(unsigned int *)pOut = i;     pOut = pOut + 4;
	//int j = i / 16, k = i % 16;
	//if( k > 0 ){ j++; }
    ...
    while( i > 0 )
	{
		...
		bool bOne =  i <= 16;
		if( bOne ){ memcpy(input, pIn, i); }
		else{ memcpy(input, pIn, 16); }
        Cipher(input, pOut);
		if( bOne ){ break; }
		i = i - 16;     pIn = pIn + 16;   pOut = pOut + 16;
	}
	//int64_t i2=time(NULL);     printf("cipher file use [%I64u] \n", (i2 - i1));
    return rzt;
}
int AES::InvCipher(const string sInput, string& sRzt)
{
    int rzt = 0;     sRzt = "";     //int64_t i1 = time(NULL);
	int i = sInput.length();
    if( i <= 4 ) return rzt;
	i = i - 4;     unsigned char* pIn = (unsigned char*)sInput.c_str();
	rzt = *(unsigned int *)pIn;     
	if( rzt <= 0 ){ return rzt; }
	sRzt.resize( rzt + 32);    pIn = pIn + 4;     unsigned char* pOut = (unsigned char*)sRzt.c_str();
    unsigned char input[16];
	while( i > 0 )
    {
        memset((void *)input,0,16);
        ...
        InvCipher(input, pOut);
		if( bOne ){ break; }
		i = i - 16;     pIn = pIn + 16;   pOut = pOut + 16;
    }
	//int64_t i2=time(NULL);     printf("InvCipher file use [%I64u] \n", (i2 - i1));
    sRzt.resize(rzt);   return rzt;
}

int AES::FileCipher(const string sFile, string& sRzt)
{
	string s = "";     int rzt = 0;
	int i = getFileBinData(sFile, s);
	if( i > 0 )
	{
		rzt = Cipher(s, sRzt);
		/*QFile fout("aaaa.rom");
		...
		{
		    fout.write((char*)sRzt.c_str(), rzt);
			fout.close();
		}*/
	}
	return rzt;
...
int AES::FileInvCipher(const string sFile, string& sRzt)
{
	string s = "";     int rzt = 0;
	int i = getFileBinData(sFile, s);
	if( i > 0 )
	{
		rzt = InvCipher(s, sRzt);
		/*QFile fout("bbbb.rom");
		if( fout.open(QIODevice::ReadWrite) )
		{
		    fout.write((char*)sRzt.c_str(), rzt);
...
void AES::FileInvCipher(QFile *fin, QFile *fout)
{
    ...
    {
        int fSz = 0;
		fin->read((char*)&fSz, sizeof(int));   //fin->seek(1);
		if( fSz <= 0 ) return;
		while(!fin->atEnd())
        {
            memset((void *)input,0,16);
            int i = fin->read((char*)input,16);
            InvCipher(input,output);
            fout->write((char*)output, i);  //16);
        }
		fout->resize(fSz);
    }
